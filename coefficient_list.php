...
/**
 *   	\file       coefficient_list.php
 *		\ingroup    funding
 *		\brief      List page for coefficient
 */
...
require_once __DIR__.'/class/coefficient.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'coefficientlist'; // To manage different context of search
...
$hookmanager->initHooks(array('coefficientlist')); // Note that conf->hooks_modules contains array
...
foreach ($object->fields as $key => $val) {
	// Fix PHP8 Edit "if ($val['searchall']) to if (isset($val['searchall']))"
...
// Extra fields
// Fix PHP8 Add "isset($extrafields->attributes[$object->table_element]['label']) && "
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Coefficient';
	$objectlabel = 'Coefficient';
...
//$help_url="EN:Module_Coefficient|FR:Module_Coefficient_FR|ES:Módulo_Coefficient";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("Coefficients"));
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/funding/coefficient_card.php', 1).'?id='.$id);
	exit;
}
...
if ($permissiontoadd) $arrayofmassactions['preenable'] = img_picto('', 'fa-check', 'class="pictofixedwidth"').$langs->trans("Enable");
if ($permissiontoadd) $arrayofmassactions['predisable'] = img_picto('', 'close_title', 'class="pictofixedwidth"').$langs->trans("Disable");
...
print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto.' infobox-contrat valignmiddle widthpictotitle pictotitle', 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendCoefficientRef";
$modelmail = "coefficient";
$objecttmp = new Coefficient($db);
...
// Fix PHP8 add "isset($extrafields->attributes[$object->table_element]['computed']) &&"
...
while ($i < ($limit ? min($num, $limit) : $num)) {
	...
	foreach ($object->fields as $key => $val) {
		...
		if (!empty($arrayfields['t.'.$key]['checked'])) {
			...
			print '</td>';
			// Fix PHP8 Add "$totalarray['nbfield'] = 0;"
			$totalarray['nbfield'] = 0;
