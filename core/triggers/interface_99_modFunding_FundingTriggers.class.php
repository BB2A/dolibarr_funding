				...
				// Regarde si il existe un lien sur une proposition
					...
					if (!empty($fudid)) {
						...
						if ($result > 0) {
							...
							if ($fudstatut > $fundingobject::STATUS_VALIDATED) {
								$result = $fundingobject->setStatusCommon($user, $fudstatut, $notrigger);
							} elseif ($fundingobject->status < $fundingobject::STATUS_VALIDATED) {
								$result = $fundingobject->setStatusCommon($user, $fundingobject::STATUS_VALIDATED, $notrigger, 'FUNDING_VALIDATE');
							}
