...
// Draft MyObject
if (!empty($conf->funding->enabled) && $user->rights->funding->funding->manage)
{
	$langs->load("orders");

	$sql = "SELECT c.rowid, c.ref, c.status, c.amount_rent_edit";
	//$sql.= ", s.code_client";
	$sql.= " FROM ".MAIN_DB_PREFIX."funding_funding as c";
	//$sql.= ", ".MAIN_DB_PREFIX."societe as s";
	//if (! $user->rights->societe->client->voir && ! $socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
	$sql.= " WHERE c.status = 1";
	$sql.= " AND c.origin = 'order'";
	...
	if ($resql)
	{
		...
		print '<th colspan="3">'.$langs->trans("FundindValidate").($num?'<span class="badge marginleftonlyshort">'.$num.'</span>':'').'</th></tr>';
		...
		if ($num > 0)
		{
			...
			while ($i < $num)
			{
				...
				print '</td>';
				print '<td class="right" class="nowrap">'.price($obj->amount_rent_edit).'</td></tr>';
