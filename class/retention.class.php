...
/**
 * \file        class/retention.class.php
 * \ingroup     funding
 * \brief       This file is a CRUD class file for Retention (Create/Read/Update/Delete)
 */
 ...
 * Class for Retention
 */
class Retention extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'funding_retention';
	...
	public $fields=array(
		...
		'fk_user_creat' => array('type'=>'integer:User:user/class/user.class.php', 'label'=>'UserAuthor', 'enabled'=>'1', 'position'=>510, 'notnull'=>1, 'visible'=>-2, 'foreignkey'=>'user.rowid', 'help'=>""),
		'fk_user_modif' => array('type'=>'integer:User:user/class/user.class.php', 'label'=>'UserModif', 'enabled'=>'1', 'position'=>511, 'notnull'=>-1, 'visible'=>-2, 'help'=>""),
		'status' => array('type'=>'smallint', 'label'=>'Status', 'enabled'=>'1', 'position'=>1000, 'notnull'=>1, 'visible'=>2, 'index'=>1, 'arrayofkeyval'=>array('0'=>'Brouillon', '1'=>'Valid&eacute;', '9'=>'Annul&eacute;'), 'help'=>""),
	);
	public $rowid;
	public $ref;
	public $fk_soc;
	...
	//  */
	// public $table_element_line = 'funding_retentionline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_retention';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Retentionline';
	...
	//  */
	// protected $childtablesoncascade = array('funding_retentiondet');

	// /**
	//  * @var RetentionLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->funding->retention->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->funding->retention->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->funding->retention->retention_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('RETENTION_VALIDATE', $user);
				if ($result < 0) $error++;
				// End call triggers
			}
		}
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'retention/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'retention/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->funding->dir_output.'/retention/'.$oldref;
				$dirdest = $conf->funding->dir_output.'/retention/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->funding->dir_output.'/retention/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'RETENTION_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'RETENTION_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'RETENTION_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Retention").'</u>';
		...
		$url = dol_buildpath('/funding/retention_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowRetention");
		...
		$hookmanager->initHooks(array('retentiondao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new RetentionLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_retention = '.$this->id));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->FUNDING_RETENTION_ADDON)) {
			$conf->global->FUNDING_RETENTION_ADDON = 'mod_retention_standard';
		}

		if (!empty($conf->global->FUNDING_RETENTION_ADDON)) {
			$mybool = false;

			$file = $conf->global->FUNDING_RETENTION_ADDON.".php";
			$classname = $conf->global->FUNDING_RETENTION_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_retention';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->RETENTION_ADDON_PDF)) {
				$modele = $conf->global->RETENTION_ADDON_PDF;
			}
		}
